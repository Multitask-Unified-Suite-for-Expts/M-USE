...
public unsafe class GLTFUtilityDracoLoader
{
	...
	public unsafe AsyncMesh LoadMesh(byte[] encodedData, MeshAttributes attributes)
	{
		...
		{
			UnityEngine.Debug.Log("Failed: Decoding error.");
			return null;
		}
	...
	public unsafe AsyncMesh CreateAsyncMesh(DracoMesh* dracoMesh, MeshAttributes attributes)
	{
		...
		mesh.verts = new UnityEngine.Vector3[dracoMesh->numVertices];
		...
		if (GetAttributeByUniqueId(dracoMesh, attributes.norms, &attr))
		{
			...
			if (GetAttributeData(dracoMesh, attr, &normData, false))
			{
				elementSize =
					...
					attr->numComponents;
				mesh.norms = new UnityEngine.Vector3[dracoMesh->numVertices];
		...
		if (GetAttributeByUniqueId(dracoMesh, attributes.uv, &attr))
		{
			...
			if (GetAttributeData(dracoMesh, attr, &texData, false))
			{
				elementSize =
					...
					attr->numComponents;
				mesh.uv = new UnityEngine.Vector2[dracoMesh->numVertices];
		...
		// Copy weights.
		UnityEngine.Vector4[] weights = null;
		if (GetAttributeByUniqueId(dracoMesh, attributes.weights, &attr))
		{
			...
			if (GetAttributeData(dracoMesh, attr, &weightData, false))
			{
				...
				if (attr->dataType == 9)
				{
					weights = new UnityEngine.Vector4[dracoMesh->numVertices];
				...
				else if (attr->dataType == 4)
				{
					...
					UnsafeUtility.MemCpy(newWeightsPtr, (void*)weightData->data,
						dracoMesh->numVertices * elementSize);
					weights = newWeightsInt.Select(x => new UnityEngine.Vector4(x.x, x.y, x.z, x.w)).ToArray();
				}
		...
		// Copy joints.
		UnityEngine.Vector4[] joints = null;
		if (GetAttributeByUniqueId(dracoMesh, attributes.joints, &attr))
		{
			...
			if (GetAttributeData(dracoMesh, attr, &jointData, false))
			{
				...
				if (attr->dataType == 9)
				{
					joints = new UnityEngine.Vector4[dracoMesh->numVertices];
				...
				else if (attr->dataType == 4)
				{
					...
					UnsafeUtility.MemCpy(newJointsPtr, (void*)jointData->data,
						dracoMesh->numVertices * elementSize);
					joints = newJointsInt.Select(x => new UnityEngine.Vector4(x.x, x.y, x.z, x.w)).ToArray();
				}
		...
		if (joints != null && weights != null)
		{
			...
			else UnityEngine.Debug.LogWarning("Draco: joints and weights not same length. Skipped");
		}

		return mesh;
	}

	public void NormalizeWeights(ref UnityEngine.Vector4 weights)
	{
